...
export default async function Pedidos() {
    ...
    return (
        <div className="flex flex-col gap-4">
            ...
            <Modal openElement={<p className="inline p-2 rounded-lg bg-red-500 text-white cursor-pointer">Insertar</p>}>
                <PedidoInsertar repartidores={repartidores} pizzas={pizzas} />
            </Modal>
            )}
            {
                pedidos.map(pedido =>
                    <div key={pedido.id} className="p-4 mb-4 bg-slate-700 rounded-lg">
                        <div className="flex flex-col gap-4">
                            ...
                            {session?.user?.role === 'ADMIN' && (
                                <>
                                    <Modal openElement={<p className="inline p-2 rounded-lg bg-red-500 text-white cursor-pointer">Modificar</p>}>
